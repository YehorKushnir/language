...
Если аудио уже создано локально, повторная генерация на VPS не нужна. Локальная
команда `pnpm audio:sync:vps` переносит MP3, `AudioAsset` и связи с контентом,
проверяет checksums и импортирует их в `morpho-learning-audio-data`.
