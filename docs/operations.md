...
`db:seed` в production публикует только версионированный контент и не создаёт локального пользователя. `publication:validate` проверяет уже сохранённое состояние PostgreSQL: маршрут, статусы, зависимости, порядок введения знаний, минимум 60 ручных упражнений на урок, русские prompts, `AnswerSpec`, покрытие знаний, шаблоны генерации и подготовленные тексты.
