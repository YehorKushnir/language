...
describe('assertLessonAvailable', () => {
  ...
  it('does not bypass prerequisites for an administrator', async () => {
  ...
  it('allows every lesson only in local development', async () => {
    vi.stubEnv('NODE_ENV', 'development')
    const prisma = createPrisma([{ prerequisiteLessonId: 'lesson.1' }], 0)

    await expect(
      assertLessonAvailable(prisma, 'user.local', 'route.1', 'lesson.2'),
    ).resolves.toBeUndefined()
    expect(prisma.courseRouteDependency.findMany).not.toHaveBeenCalled()
  })
